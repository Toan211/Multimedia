...
int main()
{
#if 1
    ofstream OutFile;
    OutFile.open("OutPut2-10k.txt"/*, ios::binary */); //output file
    if (!OutFile) {
    ...
    std::cout <<"Compressed Number of Bytes: "<< (encodedString.length())/8 << " bytes"<<endl;
     OutFile << encodedString << endl;
    ...
    OutPut << decodedString << endl; */
    return 0;
#endif

#pragma region Error Decode
#if 0
    string encodedString, decodedString;
    ifstream inFile("C:/Users/Admin/source/repos/huffman/huffman/OutPut2-10k.txt");
    string str;
    str = stream_as_string(inFile);
    calcFreq(str, str.length());
    HuffmanCodes(str.length());
